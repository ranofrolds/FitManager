...
class FuncController {
    async store(req, res){
        ...
        const { name, email, cpf, phone, category, salario, frequency, nascimento } = req.body;

        try {

            let FuncAlreadyExists = await FuncModel.findOne({ email: email });
            if (FuncAlreadyExists) return res.status(400).json({ message: "E-mail ja cadastrado" });

            FuncAlreadyExists = await FuncModel.findOne({ cpf: cpf });
            if (FuncAlreadyExists) return res.status(400).json({ message: "CPF ja cadastrado" });

            FuncAlreadyExists = await FuncModel.findOne({ phone: phone })
            if (FuncAlreadyExists) return res.status(400).json({ message: "Telefone ja cadastrado" });
            ...
            return res.status(404).json({ message: "Cadastro do Funcionario não funcionou" });
        }
    ...
    async show(req, res){
        try {
            ...
            if (!Funcs) return res.status(404).json({ message: "Funcionário não encontrado" });
    ...
    async update(req, res){
        ...
        try {
            ...
            if (!FuncUpdated) return res.status(404).json({ message: "Funcionário não encontrado" });
    ...
    async destroy(req, res){
        try {
            ...
            if (!FuncDeleted) return res.status(404).json({ message: "Funcionário não encontrado" });
